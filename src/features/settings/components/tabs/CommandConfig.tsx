...
export default function CommandConfig({
    ...
    const cmdCfg =
        (config.commandsConfig?.[cmd.name] as
            | { aliases?: string[] }
            | undefined) || {};
    const aliases =
        aliasInputs[cmd.name] !== undefined
        ...
        <div className="pb-3 border-b border-zinc-800 last:border-b-0">
                ...
                <div className="flex-1">
                    <div className="flex gap-4 items-center mb-2">
                        ...
                        <Toggle
                            checked={cmd.enabled}
                            onChange={(checked) =>
                                onToggleCommand(cmd.name, checked)
                            }
                            label="Enabled"
            ...
            <div className="mb-2">
                <MultipleInput
                    ...
                    onChange={(newAliases: string[]) => {
                        const newAliasInputs = {
                            ...aliasInputs,
                            [cmd.name]: newAliases.join("\u0000"),
                        };
                        setAliasInputs(newAliasInputs);
                        const newConfig = {
                            ...config,
                            commandsConfig: {
                                ...(config.commandsConfig as Record<
                                    string,
                                    { aliases?: string[] }
                                >),
                                [cmd.name]: {
                    ...
                    allExistingValues={(() => {
                        ...
                        if (config.commandsConfig) {
                            for (const [
                                otherCmdName,
                                otherCmdConfig,
                            ] of Object.entries(config.commandsConfig)) {
                                if (
                                    otherCmdName !== cmd.name &&
                                    (otherCmdConfig as { aliases?: string[] })
                                        .aliases
                                ) {
                                    allAliases.push(
                                        ...(
            ...
            {cmd.name === "sr" && (
                <div className="mb-3">
                    ...
                    >
                        Configure song request settings (permissions, cooldowns,
                        etc.)
                    </button>
                </div>
            )}
                        ...
                        {cmd.name === "sr" && (
                            <div className="flex flex-col gap-2 rounded-lg">
                                <Toggle
                                    ...
                                    onChange={(checked) =>
                                        onInput("replyOnCooldown", checked)
                                    }
                                    label="Reply on cooldown"
                                />
                                <Input
                                    value={
                                        typeof config.cooldownMessageTemplate ===
                                            "string" &&
                                        config.cooldownMessageTemplate !== ""
                                    ...
                                    error={
                                        !!validationErrors.cooldownMessageTemplate
                                    }
                                />
                            </div>
                        )}

                        {REPLY_CONFIGS.filter(
                            (r) =>
                                r.key === cmd.name ||
                                (cmd.name === "sr" && r.key === "songRequest")
                        ).map((replyCfg) => {
                            // Determine the default values for each template
                            const defaultReply =
                                replyCfg.key === "songRequest"
                            ...
                            const replyValue =
                                typeof config[replyCfg.replyTemplate] ===
                                    "string" &&
                                config[replyCfg.replyTemplate] !== ""
                                    ? (config[replyCfg.replyTemplate] as string)
                                    : defaultReply;
                            const errorValue =
                                typeof config[replyCfg.errorTemplate] ===
                                    "string" &&
                                config[replyCfg.errorTemplate] !== ""
                            ...
                            return (
                                ...
                                >
                                    <Toggle
                                        ...
                                        onChange={(checked) =>
                                            onInput(
                                                replyCfg.replyToggle,
                                                checked
                                            )
                                        }
                                        label={
                                            replyCfg.successLabel ||
                                            "Reply on success"
                                        }
                                    />
                                    <Input
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Toggle
                                            checked={
                                                !!config[replyCfg.errorToggle]
                                            }
                                            onChange={(checked) =>
                                                onInput(
                                                    replyCfg.errorToggle,
                                                    checked
                                                )
                                            }
                                            label={
                        ...
                        {cmd.name === "volume" && (
                            <div className="flex flex-col gap-2 rounded-lg">
                                <Toggle
                                    ...
                                    onChange={(checked) =>
                                        onInput("replyOnVolumeGet", checked)
                                    }
                                    label="Enable volume checking feature"
                                />
                                <div className="text-xs text-zinc-400 mb-2">
                                    When enabled, users can type{" "}
                                    <code className="bg-zinc-700 px-1 rounded">
                                        {config.prefix || "!"}volume
                                    </code>{" "}
                                    without arguments to get the current Spotify
                                    volume. The response will be sent to chat
                                    using the template below.
                                </div>
                                <Input
                                    value={
                                        typeof config.volumeGetReplyTemplate ===
                                            "string" &&
                                        config.volumeGetReplyTemplate !== ""
                                    ...
                                    showCharacterCount={true}
                                    error={
                                        !!validationErrors.volumeGetReplyTemplate
                                    }
                                />
                            </div>
                        )}
