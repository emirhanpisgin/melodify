        ...
        <div className="space-y-6">
            <div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between py-2">
                        <div>
                            <h3 className="text-md font-medium text-white">Minimize to system tray</h3>
                            <p className="text-sm text-gray-400">Hide the app in the system tray when minimized.</p>
                        </div>
                        <Toggle checked={config.minimizeToTray || false} onChange={handleMinimizeToTrayToggle} />
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between py-2">
                        <div>
                            <h3 className="text-md font-medium text-white">Automatic updates</h3>
                            <p className="text-sm text-gray-400">Automatically check for and download updates in the background.</p>
                        </div>
                        <Toggle checked={autoUpdateEnabled} onChange={handleAutoUpdateToggle} />
                    </div>
                    ...
                    <div className="flex justify-between items-center text-sm">
                        ...
                        <div>
                            <p className="font-medium text-white">Latest Version</p>
                            <p className="text-gray-400">{status === 'available' ? manifest?.version : currentVersion}</p>
                        </div>
                    </div>

                    {status === 'downloading' && (
                        <div>
                            <p className="text-white mb-1 text-sm">Downloading update...</p>
                            <div className="w-full bg-gray-600 rounded-full h-2">
                                <div
                                    className="bg-green-500 h-2 rounded-full transition-all"
                                    style={{ width: `${progress?.percent?.toFixed(0) ?? 0}%` }}
                                ></div>
                            </div>
                            <p className="text-xs text-gray-400 mt-1">{progress?.percent?.toFixed(0) ?? 0}% complete</p>
                        </div>
                    )}
